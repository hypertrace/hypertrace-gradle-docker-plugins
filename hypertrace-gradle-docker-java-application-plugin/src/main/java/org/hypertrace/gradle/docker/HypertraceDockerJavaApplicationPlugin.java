...
public class HypertraceDockerJavaApplicationPlugin implements Plugin<Project> {
  ...
  public static final String DOCKER_START_SCRIPT_TASK_NAME =
      "generateJavaApplicationDockerStartScript";
  ...
  private void updateDefaultJvmArgs(
      Project project, HypertraceDockerJavaApplication javaApplication) {
    ...
    project
        ...
        .setApplicationDefaultJvmArgs(
            List.of("--add-opens", "java.base/java.util.concurrent=ALL-UNNAMED"));
  }
  ...
  private TaskProvider<CreateStartScripts> getStartScriptTask(Project project) {
    return project.getTasks().named(DOCKER_START_SCRIPT_TASK_NAME, CreateStartScripts.class);
  }

  private TaskProvider<Dockerfile> getDockerfileTask(Project project) {
    return project.getTasks().named(DOCKERFILE_TASK_NAME, Dockerfile.class);
  }
  ...
  void createDockerfileTask(Project project, HypertraceDockerJavaApplication javaApplication) {
    project
        ...
        .register(
            ...
            Dockerfile.class,
            dockerfile -> {
              Provider<String> relativeScriptPath =
                  dockerfile
                      .getDestDir()
                      ...
                      .flatMap(
                          contextPath ->
                              this.getStartScriptTask(project)
              ...
              dockerfile.copyFile(
                  provideIfDirectoryExists(
                      ...
                      .map(
                          unused ->
                              new CopyFile(
                                  DOCKER_BUILD_CONTEXT_EXTERNAL_LIBS_DIR,
                                  DOCKER_BUILD_CONTEXT_EXTERNAL_LIBS_DIR)));
              dockerfile.copyFile(
                  provideIfDirectoryExists(
                          dockerfile
                      ...
                      .map(
                          unused ->
                              new CopyFile(
                                  DOCKER_BUILD_CONTEXT_ORG_LIBS_DIR,
                                  DOCKER_BUILD_CONTEXT_ORG_LIBS_DIR)));
              dockerfile.copyFile(
                  provideIfDirectoryExists(
                          dockerfile
                      ...
                      .map(
                          unused ->
                              new CopyFile(
                                  DOCKER_BUILD_CONTEXT_LOCAL_LIBS_DIR,
                                  DOCKER_BUILD_CONTEXT_LOCAL_LIBS_DIR)));
              dockerfile.copyFile(
                  provideIfDirectoryExists(
                          dockerfile
                      ...
                      .map(
                          unused ->
                              new CopyFile(
                                  DOCKER_BUILD_CONTEXT_RESOURCES_DIR,
                                  DOCKER_BUILD_CONTEXT_RESOURCES_DIR)));
              dockerfile.copyFile(
                  new CopyFile(DOCKER_BUILD_CONTEXT_CLASSES_DIR, DOCKER_BUILD_CONTEXT_CLASSES_DIR));
              dockerfile.copyFile(
                  relativeScriptPath.map(relativePath -> new CopyFile(relativePath, "run")));
              ...
              dockerfile.label(
                  javaApplication.maintainer.map(
                      maintainer -> singletonMap("maintainer", maintainer)));
  ...
  private void createDockerStartScriptTask(Project project) {
    project
        .getTasks()
        .register(
            DOCKER_START_SCRIPT_TASK_NAME,
            CreateStartScripts.class,
            startScript -> {
              startScript.getInputs().file(this.getStartScriptTemplate(project));
              ((TemplateBasedScriptGenerator) startScript.getUnixStartScriptGenerator())
                  .setTemplate(this.getStartScriptTemplate(project));
              startScript.setGroup(DockerPlugin.TASK_GROUP);
              startScript.setDescription(
                  "Creates a startup script for use by the docker container");
              startScript.mustRunAfter(SYNC_BUILD_CONTEXT_TASK_NAME);

              Directory contextDir = project.getLayout().getBuildDirectory().dir("docker").get();
              JavaApplication javaApplication =
                  project.getExtensions().getByType(JavaApplication.class);
              startScript.getMainClass().set(javaApplication.getMainClass());
              startScript.setApplicationName(javaApplication.getApplicationName());
              startScript.setOutputDir(
                  contextDir.file(DOCKER_BUILD_CONTEXT_SCRIPTS_DIR).getAsFile());
              List<String> jvmArgs = new ArrayList<>();
              javaApplication.getApplicationDefaultJvmArgs().forEach(jvmArgs::add);
              // add generational zgc if it is java 21 based
              if (getHypertraceDockerApplicationExtension(project).javaVersion.get()
    ...
    project
        ...
        .register(
            ...
            sync -> {
              ...
              sync.into(this.getDockerfileTask(project).map(Dockerfile::getDestDir).get());
              sync.with(
                  project.copySpec(
                      spec -> {
                        spec.into(
                            ...
                            childSpec ->
                                childSpec.from(
                                    this.buildFilteredLibraryProvider(
                                        project, this.buildLocalArtifactPredicate(project))));
                        spec.into(
                            DOCKER_BUILD_CONTEXT_ORG_LIBS_DIR,
                            childSpec ->
                                childSpec.from(
                                    this.buildFilteredLibraryProvider(
                                        project,
                                        this.buildLocalArtifactPredicate(project)
                                            .negate()
                            ...
                            childSpec ->
                                childSpec.from(
                                    this.buildFilteredLibraryProvider(
                                        ...
                                        this.buildLocalArtifactPredicate(project)
                                            .negate()
                                            .and(this.getOrgLibPredicate(project).negate()))));
                        spec.into(
                            DOCKER_BUILD_CONTEXT_CLASSES_DIR,
                            childSpec ->
                                childSpec.from(mainSourceSetOutput(project).getClassesDirs()));
                        spec.into(
                            DOCKER_BUILD_CONTEXT_RESOURCES_DIR,
                            childSpec ->
                                childSpec.from(mainSourceSetOutput(project).getResourcesDir()));
                      }));
            });
  }
  ...
  private Predicate<ResolvedArtifact> buildLocalArtifactPredicate(Project project) {
    ...
    return artifact -> {
      ...
      return artifactVersion.equals(project.getVersion().toString())
          || artifactVersion.equals("unspecified");
    };
  }

  private Configuration getRuntimeClasspath(Project project) {
    return project.getConfigurations().getByName(RUNTIME_CLASSPATH_CONFIGURATION_NAME);
  }
  ...
  private Provider<File> provideIfDirectoryExists(Provider<Directory> directoryProvider) {
    return directoryProvider.map(
        directory -> directory.getAsFile().isDirectory() ? directory.getAsFile() : null);
  }

  private TextResource getStartScriptTemplate(Project project) {
    URL resourceUrl =
        HypertraceDockerJavaApplicationPlugin.class
            .getClassLoader()
            .getResource("application-start-script.template.sh");
    return project.getResources().getText().fromUri(requireNonNull(resourceUrl));
  }

  private Provider<Collection<File>> buildFilteredLibraryProvider(
      Project project, Predicate<ResolvedArtifact> filterPredicate) {
    return project.provider(
        () ->
            this.getRuntimeClasspath(project)
